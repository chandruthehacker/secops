...
import { Search, ShieldAlert, CheckSquare, Clock, Target, ChevronDown, UserCheck, XCircle, CheckCircle2, AlertTriangle, Square, Loader2 } from 'lucide-react';
...
export default function AlertQueuePage() {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: AlertStatus }) => alertsApi.updateStatus(id, status),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['alerts'] }),
  });

  const assignMutation = useMutation({
    mutationFn: ({ id, userId }: { id: string; userId: string }) => alertsApi.assign(id, userId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['alerts'] }),
  });
  ...
  const getAssigneeName = (assigneeId: string | undefined) => {
    ...
    const u = analysts.find(a => a.id === assigneeId);
    ...
    <MainLayout>
      <div className="flex flex-col gap-5">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {isFetching && !isLoading && <Loader2 className="w-5 h-5 animate-spin text-muted-foreground" />}
        </div>

        {/* Status Tabs */}
        <div className="flex items-center gap-1 bg-secondary/50 p-1 rounded-xl border border-border w-fit flex-wrap">
        ...
        <div className="flex gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[280px]">
            ...
            <input
              type="text"
              placeholder="Search alerts by title, rule, IP..."
              value={searchTerm}
        ...
        <div className="bg-card border border-border rounded-xl shadow-lg shadow-black/20 flex flex-col overflow-hidden">
              ...
              <div className="flex gap-2 ml-auto flex-wrap">
                ...
                {canAssign && analysts.length > 0 && (
                  <div className="relative">
                    ...
                    {bulkAssignOpen && (
                      <div className="absolute right-0 top-full mt-1 bg-card border border-border rounded-lg shadow-xl z-20 py-1 min-w-[160px] max-h-48 overflow-y-auto">
                        {analysts.map(a => (
                          <button key={a.id} onClick={() => bulkAssign(a.id)} className="w-full text-left px-3 py-2 text-sm text-foreground hover:bg-secondary transition-colors">
            ...
            {isLoading ? (
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-border/40">
                  {alerts.map(alert => {
                    const assigneeName = getAssigneeName(alert.assignee);
                    return (
                      <tr key={alert.id} className={`hover:bg-secondary/40 transition-colors group ${selectedIds.has(alert.id) ? 'bg-primary/5' : ''}`}>
                        ...
                        <td className="px-4 py-3.5 font-mono text-xs text-muted-foreground">
                          <Link href={`/alerts/${alert.id}`} className="hover:text-primary hover:underline">{alert.id.slice(0, 8)}</Link>
                        </td>
                        <td className="px-4 py-3.5 font-medium text-foreground max-w-xs">
                          <Link href={`/alerts/${alert.id}`} className="hover:text-primary transition-colors block">
                            ...
                            <span className="text-xs text-muted-foreground font-normal mt-0.5 flex items-center gap-1">
                              <Target className="w-3 h-3" /> {alert.mitreTactics[0] || alert.ruleName || 'Unknown'}
                            </span>
                          </Link>
                        </td>
                        ...
                        <td className="px-4 py-3.5">
                          {assigneeName ? (
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm text-foreground truncate max-w-[90px]">{assigneeName}</span>
                            </div>
                          ) : (
                        ...
                        <td className="px-4 py-3.5 text-right">
                          <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            {canClose && alert.status !== 'resolved' && (
                              <button onClick={e => handleQuickAction(e, alert.id, 'resolved')} title="Resolve" className="p-1.5 text-emerald-400 hover:bg-emerald-400/10 rounded-md transition-colors">
                                <CheckCircle2 className="w-4 h-4" />
                              </button>
                            )}
                            {canTriage && alert.status === 'new' && (
                              <button onClick={e => handleQuickAction(e, alert.id, 'investigating')} title="Investigate" className="p-1.5 text-amber-400 hover:bg-amber-400/10 rounded-md transition-colors">
                                <AlertTriangle className="w-4 h-4" />
                              </button>
                            )}
                            <Link href={`/alerts/${alert.id}`} className="px-2.5 py-1.5 bg-primary/10 text-primary hover:bg-primary/20 font-medium rounded-md transition-colors text-xs">
                              View
                            </Link>
                          </div>
                        </td>
          ...
          <div className="p-3 border-t border-border bg-secondary/20 text-xs text-muted-foreground">
            Showing {alerts.length} of {data?.total ?? 0} alerts
          </div>
        </div>
      </div>
    </MainLayout>
