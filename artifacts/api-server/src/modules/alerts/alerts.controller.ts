...
export async function updateStatus(req: Request, res: Response): Promise<void> {
  ...
  const alert = await alertsService.updateAlertStatus(req.params.id!, status, req.user!.userId);
  if (!alert) { res.status(404).json({ error: "Alert not found" }); return; }
...
export async function assignAlert(req: Request, res: Response): Promise<void> {
  ...
  const alert = await alertsService.assignAlert(req.params.id!, assignedTo, req.user!.userId);
  if (!alert) { res.status(404).json({ error: "Alert not found" }); return; }
